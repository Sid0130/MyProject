...
// import static 문장 - 클래스(인터페이스, enum)에서 static으로 선언된 필드/메서드 이름 import.
import static com.itwill.contact.ver04.util.FileUtil.*; // '*' 모든 필드/메서드 다 가지고 온다.
...
public enum ContactDao {
	...
	ContactDao() {
		// (1)데이터 폴더 초기화: 폴더가 없는 경우에 폴더를 새로 만듦.
		initializeDataDir(); 
		// (2)연락처 데이터 초기화: 데이터 파일에서 리스트를 읽어 오거나, 또는 빈 리스트를 생성. 
		contacts = initializeData(); // 저장된 파일에서 연락처를 가지고 오겠다.
	}
	 ...
	 * @return (0 <= index < size()) 범위에 있으면 true, 그렇지 않으면 false.
	 */
	 ...
	 * @return 성공하면 1, 실패하면 0.
	 */
	public int updata(int index, Contact contact) { // 메인에서 입력 받은 정보들.
		if(!isValidIndex(index)) {
			return 0;
		} 
		// 특정 index 위치의 연락처 정보를 수정
//		contacts.set(index, contact);
		Contact prvious = contacts.get(index);
		prvious.setName(contact.getName());
		prvious.setPhone(contact.getPhone());
		prvious.setEmail(contact.getEmail());
	...
	public int delete(int index) {
		...
		writeDataToFile(contacts); // 변경된 리스트를 파일에 쓴다.
		return 1;
	}
