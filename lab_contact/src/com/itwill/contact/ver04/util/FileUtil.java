...
// 도우미 클래스(helper class): 파일 관련(일기, 쓰기, 폴더 생성) 기능을 제공하는 클래스.
...
public class FileUtil {
	...
	/**
	 * 연락처 데이터 파일을 저장하는 폴더가 만들어져 있지 않으면 폴더를 새로 생성하고, File 타입의 객체 리턴, 
	...
	public static File initializeDataDir() {
		...
		if (dir.exists()) {
			...
			if (result) {
				...
				System.out.println("데이터 폴더 생설 실패!");
				dir = null;
			}
		}
		return dir;
	...
	public static List<Contact> initializeData(){
		List<Contact> list = new ArrayList<>(); // 파일이 없으면 비어있는 리스트가 리턴이 됨.
		
		File file = new File(DATA_DIR, DATA_FILE); 
		if(file.exists()) { // 파일이 존재하면 기존 값 무시하고 새로 할당 - 파일에서 데이터를 읽어서 리턴
